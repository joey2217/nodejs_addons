...
public:
    ...
    int32_t compare(const ch_t* rhs, size_t rhs_len) const {
        if (!data() && !rhs) {
            return 0;
        }
        if (!data()) {
        ...
        if (result == 0) {
            result = static_cast<int32_t>(length()) - static_cast<int32_t>(rhs_len);
        }
        return result;
    }
...
CubeString operator+(const CubeString::ch_t*, const CubeString& rh);
